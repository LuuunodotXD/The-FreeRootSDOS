...
FS_SECS=2048
DISK_SECS=2880                          # tamanho total da imagem (1,44MB, com folga)
...
# ------------------------------------------------------------
# 3. Bootloader Stage 2 (loader.asm — mini-shell)
# ------------------------------------------------------------
...
if [ "$SIZE2" -gt "$MAXSIZE2" ]; then
    echo "ERRO: loader.bin tem $SIZE2 bytes, mas o limite e $MAXSIZE2 ($STAGE2_SECS setores)."
    echo "Aumente STAGE2_SECS neste script e em boot/superbloco.inc."
    exit 1
fi

# Preenche com zeros até completar STAGE2_SECS setores exatos
...
# ------------------------------------------------------------
# 5. Compila todo o código C do kernel
# ------------------------------------------------------------
...
# ------------------------------------------------------------
# 6. Linka tudo (entry.o sempre primeiro — garante _start em 0x10000)
# ------------------------------------------------------------
echo "==> Linkando kernel.elf"
OBJS=$(ls *.o | grep -v '^entry\.o$')
i686-linux-gnu-ld -m elf_i386 -T linker.ld -o kernel.elf entry.o $OBJS
...
if [ "$KSIZE" -gt "$MAXKSIZE" ]; then
    echo "ERRO: kernel.bin tem $KSIZE bytes, mas o limite e $MAXKSIZE ($KERNEL_SECS setores)."
