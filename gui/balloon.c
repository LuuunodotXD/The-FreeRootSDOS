...
static void draw_icon(int i) {
    ...
    switch (i) {
        ...
        case 4: draw_icon_img_mines   (ix, iy); break;   // era editor
        case 5: draw_icon_img_editor  (ix, iy); break;   // era mines
...
// ----------------------------------------------------------------
// Dock (barra de aplicativos flutuante)
// ----------------------------------------------------------------
#define DOCK_ICON_W        ICON_IMG_W
#define DOCK_ICON_H        ICON_IMG_H
#define DOCK_PAD_X         8
#define DOCK_PAD_Y         6
#define DOCK_GAP           6
#define DOCK_MARGIN_BOTTOM 4

#define DOCK_W  (ICON_COUNT*DOCK_ICON_W + (ICON_COUNT-1)*DOCK_GAP + DOCK_PAD_X*2)
#define DOCK_H  (DOCK_ICON_H + DOCK_PAD_Y*2 + 4)
#define DOCK_X  ((VGA12_W - DOCK_W) / 2)
#define DOCK_Y  (VGA12_H - DOCK_H - DOCK_MARGIN_BOTTOM)

static int dock_icon_x(int i) {
    return DOCK_X + DOCK_PAD_X + i * (DOCK_ICON_W + DOCK_GAP);
}
static int dock_icon_y(void) {
    return DOCK_Y + DOCK_PAD_Y;
}

// Procura janela aberta correspondente ao ícone idx (mesma lógica de dedupe usada em icon_open_window)
static int find_window_for_icon(int idx) {
    const char *r = icon_defs[idx].win_title;
...
static void draw_dock(void) {
    vga12h_rect  (DOCK_X, DOCK_Y, DOCK_W, DOCK_H, C_LGRAY);
    vga12h_border(DOCK_X, DOCK_Y, DOCK_W, DOCK_H, C_DGRAY);
    vga12h_hline (DOCK_X + 1, DOCK_Y + 1, DOCK_W - 2, C_WHITE);

    for (int i = 0; i < ICON_COUNT; i++) {
        int ix = dock_icon_x(i);
        int iy = dock_icon_y();
        int win = find_window_for_icon(i);

        if (win >= 0)
            vga12h_rect(ix - 2, iy - 2, DOCK_ICON_W + 4, DOCK_ICON_H + 4, C_LBLUE);
        ...
        // Indicador de janela aberta (bolinha embaixo do ícone)
        if (win >= 0) {
            int dx = ix + DOCK_ICON_W / 2 - 1;
            int dy = DOCK_Y + DOCK_H - 4;
            vga12h_rect(dx, dy, 3, 2, C_BLACK);
        }
    }
}

static int hit_dock(int mx_, int my_) {
    if (mx_ < DOCK_X || mx_ >= DOCK_X + DOCK_W) return -1;
    if (my_ < DOCK_Y || my_ >= DOCK_Y + DOCK_H) return -1;
    for (int i = 0; i < ICON_COUNT; i++) {
        int ix = dock_icon_x(i);
        if (mx_ >= ix && mx_ < ix + DOCK_ICON_W) return i;
    }
    return -1;
}
...
BalloonWin balloon_open(const char *title, int x, int y, int w, int h,
    ...
    windows[i].title[j] = '\0';
    num_windows++;
    sound_note_on(0, 784, 15, SOUND_PIANO);
    balloon_redraw();
    return i;
}
...
void balloon_close(BalloonWin id) {
    ...
    sound_note_on(0, 523, 15, SOUND_PIANO);
    balloon_redraw();
}
...
void balloon_run(void) {
	...
	if ((btn & MOUSE_BTN_RIGHT) && !(btn_prev & MOUSE_BTN_RIGHT)) {
	    ...
	    for (int i = BALLOON_MAX_WINDOWS - 1; i >= 0; i--) {
            ...
            // 4.5. Cliques na dock
            {
                int di = hit_dock(mx_, my_);
                if (di >= 0) {
                    int win = find_window_for_icon(di);
                    if (win >= 0) {
                        windows[win].minimized = 0;
                        bring_to_front(win);
                    } else {
                        icon_open_window(di);
                    }
